  ...
  describe("Utils", () => {
    describe("validateDomain", () => {
      ...
      it.each(
        ...
        // the char code alone: interpolating the character itself puts control characters and
        // non-characters like U+FFFE in the test name, which jest-sonar escapes into XML
        // character references that are illegal in XML, breaking the SonarQube report
        "should accept or reject a domain depending on if it's containing ASCII or non ASCII caracters. Testing char code $code",
        char => {
